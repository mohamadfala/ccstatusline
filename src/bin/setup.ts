...
export function patchSettings(opts: SetupOpts = {}): { written: boolean; backup?: string } {
  ...
  if (existsSync(file)) {
    ...
    try {
      ...
      throw new Error(
        `existing ${file} is not valid JSON; refusing to overwrite (${(err as Error).message})`
      );
    }
...
if (import.meta.url === `file://${process.argv[1]}`) {
  try {
    ...
    process.stdout.write(
      `Run /ccstatusline-config to customize. Restart Claude Code or wait for the next interaction to see the status line.\n`
    );
  } catch (err) {
